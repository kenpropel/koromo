module Koromo
  ...
  class Handler
    ...
    def self.autoload
      Koromo.config.handler_paths.each { |path|
        load_from_path(path)
      }
    end
    ...
    # Call as often as necessary to add handlers with blocks; each call creates an Koromo::Handler object
    ...
    def self.add(type, name = nil, &block)
      ...
      Koromo.logger.debug("Added #{type} handler: #{@handlers[type].last}")
    end
